...
### ⚠️ ตามเก็บ 2026-09-09 — 2 ตารางหลุด RLS (Supabase Security Advisor เตือน)

Supabase ส่งอีเมล `rls_disabled_in_public` มา — ไล่ดูแล้วเจอว่า **`bankReconBook` / `bankReconMatch`
ค้าง "ปิด RLS"** มาตั้งแต่ตอนตั้งระบบ เพราะลำดับนี้:

1. `bankrecon-express.sql` เปิด RLS ให้ 2 ตารางนี้ (แต่ policy สร้างไม่สำเร็จ — บล็อกนั้นเรียก
   `public.auth_role()` ที่ยังไม่มี แล้วดักด้วย `exception when others then null` ⇒ เงียบ)
2. `rls-off-bootstrap.sql` ปิด RLS **ทุกตารางใน public** แบบวนลูป ⇒ 2 ตัวนี้โดนปิดไปด้วย
3. `rls-phase4.sql` เปิดคืนจาก **รายชื่อ hardcode ที่ไม่มี 2 ตัวนี้** ⇒ ค้างเปิดโล่งถึง live

ผล: ใครถือ anon key (อยู่ใน repo public) อ่าน/เขียน/ลบ 2 ตารางนี้ได้ · ตารางเป็น**ตารางเปล่า**
(BDH ตัดหน้ากระทบยอดออก) ไม่มีข้อมูลการเงินหลุด แต่ "เขียนได้" = ยัดขยะเข้ามาได้

**แก้:** รัน `supabase/rls-fix-bankrecon.sql` แล้วดูผลคิวรีท้ายไฟล์ (ต้องได้ no rows ทั้ง 2 อัน)
พร้อมกันนี้เติมชื่อ 2 ตารางเข้า `rls-phase4.sql` (ทั้งรายชื่อหลักและ ROLLBACK block) แล้ว

> **บทเรียน:** `rls-off-bootstrap.sql` ปิดแบบ "วนลูปทุกตาราง" แต่ `rls-phase4.sql` เปิดคืนแบบ
> "รายชื่อ hardcode" — สองไฟล์ไม่สมมาตรกัน ⇒ **เพิ่มตารางใหม่เมื่อไหร่ ต้องเติมชื่อใน
> `rls-phase4.sql` ด้วยเสมอ** ไม่งั้นหลุดเงียบ ๆ · ตรวจซ้ำได้ด้วยคิวรีท้าย `rls-fix-bankrecon.sql`
