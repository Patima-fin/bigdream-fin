// BIGDREAM Financial Dashboard – App shell, sidebar nav, routing, tweaks integration.
...
(function () {
  try {
    if (localStorage.getItem('bdh-proj-resync-v1') === '1') return;
    var raw = localStorage.getItem('bdh-fin-data-v8');
    if (raw) {
      var c = JSON.parse(raw);
      if (c && Array.isArray(c.projects)) { c.projects = []; localStorage.setItem('bdh-fin-data-v8', JSON.stringify(c)); }
    }
    localStorage.removeItem('bdh-proj-control-v2'); // ทิ้ง snapshot เก่า → ใช้ data.projects (synced 648) แทน
    localStorage.setItem('bdh-proj-resync-v1', '1');
  } catch (_) {}
})();
...
// ★ BIGDREAM = 10 หน้า (BIO มี 30+ · ที่เหลือถูกตัดออกทั้งไฟล์ ดู index.html)
//   `data_bank` (DATA BANK · บัญชีธนาคาร) ไม่อยู่ในเมนูแล้ว — ย้ายไปเป็น "แท็บ"
//   ในหน้า `daily_balance` (บันทึกยอดธนาคาร) ตามที่ตกลงไว้
const PAGE_GROUPS = [
  { key: 'dash', label: 'แดชบอร์ด', items: [
    ['cashflow_present', 'Executive Cash Flow',   'chart'],
  ] },
  ...
  { key: 'manage', label: 'จัดการข้อมูล', items: [
    ['invoices',      'ใบแจ้งหนี้คงค้าง',        'invoice'],
    ['data_pv',       'DATA PV',                 'money'],
    ['data_payable',  'DATA AP Outstanding',     'arrow_up'],
...
const ROLE_PERMS = {
  // ผู้บริหาร — เห็นเฉพาะ Executive Cash Flow Dashboard
  viewer: {
    pages: new Set(['cashflow_present']),
  ...
  // เจ้าของ — ดูได้ทุกหน้า แต่แก้/ลบไม่ได้ + ไม่เห็น audit + users + บันทึกยอดธนาคาร
  owner: {
    pages: '*', excludePages: new Set(['users', 'audit_log', 'backup', 'daily_balance', 'bank_diary']),
    canEdit: false, canDelete: false, canApprove: false, canManageUsers: false,
  },
};
function _getSession() {
  try { return JSON.parse(localStorage.getItem('bdh-session') || 'null'); } catch { return null; }
}
function _getRole() {
  const s = _getSession();
...
window.WTPAuth = {
  ...
  firstAllowedPage(allRoutes) {
    return (allRoutes || PAGE_ORDER).find(r => this.canViewPage(r)) || 'cashflow_present';
  },
};
...
function App() {
  const [isLoggedIn, setIsLoggedIn] = aState(() => {
    try {
      const s = JSON.parse(localStorage.getItem('bdh-session') || 'null');
      if (!s) return false;
      const ttl = (window.WTP_CONFIG && window.WTP_CONFIG.SESSION_TTL_MS) || 0;
      if (ttl > 0 && Date.now() - s.time > ttl) { localStorage.removeItem('bdh-session'); return false; }
      ...
      if (flb > 0 && s.time && s.time < flb) { localStorage.removeItem('bdh-session'); return false; }
      return true;
    } catch { return false; }
  });
  ...
  const handleLogin = (userObj) => {
    ...
    localStorage.setItem('bdh-session', JSON.stringify(session));
    setCurrentUser(session);
    setIsLoggedIn(true);
    try { WTPData.logEvent && WTPData.logEvent('login', { actor: session, summary: 'เข้าสู่ระบบ' }); } catch (_) {}
    // ทุกคนที่ login เข้ามาต้องเจอหน้า Executive Cash Flow ก่อนเสมอ (ไม่ว่า hash เดิมจะค้างหน้าไหน)
    try { window.location.hash = '#cashflow_present'; } catch (_) {}
    setRoute('cashflow_present');
  };
  const handleLogout = () => {
    ...
    localStorage.removeItem('bdh-session');
    setIsLoggedIn(false);
    setCurrentUser(null);
  };
  ...
  aEffect(() => {
    ...
    if (localStorage.getItem('bdh-override-migrated-v1') === '1') {
      migratedRef.current = true;
      return;
    }
    try {
      const local = JSON.parse(localStorage.getItem('bdh-manual-overrides') || '{}');
      const localKeys = Object.keys(local);
      if (localKeys.length === 0) {
        localStorage.setItem('bdh-override-migrated-v1', '1');
        migratedRef.current = true;
        return;
      }
      ...
      let updatedBy = '';
      try { updatedBy = (JSON.parse(localStorage.getItem('bdh-session') || 'null') || {}).username || ''; } catch (_) {}
  ...
  aEffect(() => {
    ...
    const checkIdle = () => {
      const now = Date.now();
      let sessTime = 0;
      try { sessTime = (JSON.parse(localStorage.getItem('bdh-session') || 'null') || {}).time || 0; } catch (_) {}
  ...
  aEffect(() => {
    ...
    const beat = () => {
      ...
      let s = null; try { s = JSON.parse(localStorage.getItem('bdh-session') || 'null'); } catch (_) {}
  ...
  const [sbCollapsed, setSbCollapsed] = aState(() => {
    try { return localStorage.getItem('bdh-sb-collapsed') === '1'; } catch (_) { return false; }
  });
  const toggleCollapse = () => setSbCollapsed(c => {
    const next = !c;
    try { localStorage.setItem('bdh-sb-collapsed', next ? '1' : '0'); } catch (_) {}
    return next;
  });
  ...
  aEffect(() => {
    ...
    try { localStorage.setItem('bdh-session', JSON.stringify(merged)); } catch (_) {}
  ...
  aEffect(() => {
    ...
    // Theme palette
    const themes = {
      // ★ BIGDREAM: เขียว hue เดียวกับ BIO แต่ไล่เฉด "อ่อนขึ้น ~1.5 เฉด" ทั้งชุด
      //   (BIO เดิม: 500 #2e8b4a · 600 #21703a · 700 #1a592f · 400 #47a566 · 100 #d5edda · 50 #eef7f1)
      //   ปุ่มหลักชดเชยด้วยการไล่ 500→700 ใน .btn-primary — ดู styles.css
      green:   { 500: '#3ea45f', 600: '#2d8b4d', 700: '#22703c', 800: '#1a5730', 400: '#62bd85', 300: '#94d4aa', 200: '#bfe6cc', 100: '#e0f4e7', 50: '#f3faf5' }, blue:    { 500: '#2a6fdb', 600: '#1f56b8', 700: '#1a4490', 800: '#16356f', 400: '#5b94f7', 300: '#8db8ff', 200: '#b9d4ff', 100: '#dceaff', 50: '#f0f6ff' },
  ...
  const routes = {
    ...
    invoices:      { label: 'ใบแจ้งหนี้คงค้าง',       title: 'Invoices',         icon: 'invoice' },
    data_pv:       { label: 'DATA PV',               title: 'Payment Vouchers', icon: 'money' },
    data_payable:  { label: 'DATA AP Outstanding',   title: 'ใบแจ้งหนี้เจ้าหนี้คงค้าง', icon: 'invoice' },
    daily_balance: { label: 'บันทึกยอดธนาคาร',       title: 'บันทึกยอดธนาคารรายวัน', icon: 'bank' },
    audit_log:     { label: 'Audit Log',             title: 'Audit Log · บันทึกการแก้ไขข้อมูล', icon: 'settings' },
  ...
  switch (route) {
    ...
    // DATA BANK ไม่มี route ของตัวเองแล้ว — เป็นแท็บใน DailyBalancePage
    ...
    default:               page = <CashFlowPresentPage data={data} setData={setData} toast={pushToast} />;
  }
  ...
  return (
    <aside className={`sb ${isOpen ? 'is-open' : ''} ${collapsed ? 'is-collapsed' : ''}`}>
      <div className="sb-brand" style={{ position: 'relative' }}>
        {/* โลโก้เต็ม (เมนูกาง) ↔ สัญลักษณ์จัตุรัส (เมนูย่อ) — สลับด้วย CSS ใน .sb.is-collapsed */}
        <img src="bigdream_logo.png" alt="BIG DREAM HOLDINGS" className="sb-logo-img" />
        <img src="bigdream_mark.png" alt="BIG DREAM HOLDINGS" className="sb-logo-mark" />
...
function PresentModeToggle() {
  const [on, setOn] = aState(() => { try { return localStorage.getItem('bdh-present-mode') === '1'; } catch (_) { return false; } });

  // สไตล์ + persist ตามสถานะ on
  aEffect(() => {
    document.body.classList.toggle('present-mode', on);
    try { localStorage.setItem('bdh-present-mode', on ? '1' : '0'); } catch (_) {}
  }, [on]);
    ...
    <div className="topbar">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
        ...
        <div className="crumbs">
          <span>BIGDREAM</span><span className="sep">/</span>
...
function LoginPage({ onLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    const cachedUsers = () => { try { const c = JSON.parse(localStorage.getItem('bdh-fin-data-v8') || 'null'); return (c && Array.isArray(c.users)) ? c.users : []; } catch (_) { return []; } };
  ...
  return (
    <div style={{
      ...
      // เดิมเป็นฟ้าที่ตกทอดมาจาก Water POG → เปลี่ยนเป็นเขียวอ่อนชุด BIGDREAM
      background: 'linear-gradient(145deg, #e3f3e9 0%, #f5faf6 55%, #e9f6ee 100%)',
      ...
      <div style={{
        ...
        boxShadow: '0 24px 64px rgba(45,139,77,0.13), 0 4px 16px rgba(0,0,0,0.06)',
        border: '1px solid rgba(45,139,77,0.10)',
      }}>
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          {/* โลโก้แนวนอน 4:1 → คุมด้วยความกว้าง (คุมด้วย height 88 จะกว้างเกินการ์ด) */}
          <img src="bigdream_logo.png" alt="BIG DREAM HOLDINGS"
            style={{ width: 250, maxWidth: '86%', height: 'auto', objectFit: 'contain', margin: '0 auto 10px', display: 'block' }} />
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            ...
            <input
              ...
              style={inputStyle}
              onFocus={e => e.target.style.borderColor = '#3ea45f'}
          ...
          <button type="submit" disabled={loading} style={{
            ...
            // สีชุด BIGDREAM (อ่อนกว่า BIO ~1.5 เฉด) — hardcode เพราะหน้า login เรนเดอร์
            // ก่อน effect ที่เซ็ต --brand-* จะทำงาน ⇒ ใช้ตัวแปรไม่ได้ ต้องแก้ที่นี่ด้วยถ้าเปลี่ยนธีม
            background: loading ? '#a9dcb9' : 'linear-gradient(135deg, #3ea45f, #22703c)',
            color: '#fff', border: 'none', borderRadius: 11,
            fontSize: 15, fontWeight: 600,
            cursor: loading ? 'not-allowed' : 'pointer',
            boxShadow: loading ? 'none' : '0 4px 16px rgba(34,112,60,0.32)',
