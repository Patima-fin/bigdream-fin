/* =====================================================================
 * Cash Flow Presentation (#cashflow_present) — BIGDREAM
 * ---------------------------------------------------------------------
 ...
 *    → ทุกคน/ผู้บริหารเห็นชุดเดียวกัน. localStorage `bdh-cfpresent-v1` = cache/offline
 *    ต่อเครื่อง. ต้องรัน supabase/cashflow-present.sql ครั้งเดียวก่อน (ไม่งั้น degrade เป็น local).
 * ===================================================================== */
(function () {
  const { useState, useEffect, useMemo, useRef } = React;
  const CFP_LS = 'bdh-cfpresent-v1';
  ...
  function cfpCurrentUser() { try { var s = JSON.parse(localStorage.getItem('bdh-session') || 'null'); return s ? (s.displayName || s.username || '') : ''; } catch (e) { return ''; } }
  ...
  const C = {
    primary: '#3ea45f', primaryD: '#2b8a4c', teal: '#1aa46f', purple: '#9b7bff',
    ...
    shadow: '0 10px 30px rgba(45,139,77,.15)',
  };
  ...
  const ACT_COLOR = { op: '#3ea45f', inv: '#e08a3c', fin: '#9b7bff', transfer: '#688275', other: '#688275' };
  const ACT_TAGBG = { op: '#e4f2e9', inv: '#fff0e6', fin: '#efe8ff', transfer: '#eef2ef', other: '#eef2ef' };
  const ACT_TAGFG = { op: '#2d8b4d', inv: '#d98032', fin: '#7a5fd0', transfer: '#688275', other: '#688275' };
   ...
   *   สีคงชุดแบรนด์ BIGDREAM: เขียว = การ์ดหลัก/เงินเข้า · แดง = เงินออก
  ...
  function CfpHeroBoard({ model }) {
    ...
    return (
      <div className="cfp-hero-grid">
        ...
        <div className="cfp-card cfp-hero-end" style={{ position: 'relative', overflow: 'hidden', borderRadius: 18, padding: '18px 22px', color: '#fff', background: 'linear-gradient(140deg,#34a86e 0%,#1f7d46 52%,#134d2c 100%)', boxShadow: '0 14px 36px rgba(45,139,77,.32)' }}>
  ...
  // ค่าเริ่มต้น BIGDREAM (จับด้วยเลขท้ายบัญชี 4 หลัก) — แก้ทับได้ผ่าน ✏️ จัดประเภทบัญชี (sync ทั้งทีม)
  ...
  function CfpCashUsable({ model, acctTypes, canEdit, onSave }) {
    ...
    if (!model.hasAcctBalances || !accts.length) {
      ...
      return (
        <div className="cfp-card no-print no-present" style={{ ...cfpCard, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: 'linear-gradient(135deg,rgba(62,164,95,.07),rgba(31,110,58,.10))' }}>
        ...
        {mo.map((d, i) => {
          ...
          return (
            <g key={'g' + i} style={{ cursor: 'pointer' }} onClick={() => onPick && onPick(d.m)}>
              <rect x={cx(i) - slot / 2 + 3} y={padTop - 8} width={slot - 6} height={plotH + 8} rx="9" fill={i % 2 ? 'rgba(62,164,95,0.045)' : 'transparent'} />
              {acts.map((k, j) => {
  ...
  function CashFlowPresentPage({ data, setData, toast }) {
    ...
    const [topN, setTopN] = useState(10); const [era, setEra] = useState(() => { try { return localStorage.getItem('bdh-cfp-era') || 'auto'; } catch (e) { return 'auto'; } });
    const [modal, setModal] = useState(null);
    const [synced, setSynced] = useState(false);      // โหลด/แชร์ผ่านส่วนกลาง (Supabase) สำเร็จล่าสุด
    const [shareBusy, setShareBusy] = useState(false);
    const [orient, setOrient] = useState(() => { try { return localStorage.getItem('bdh-cfp-print-orient') || 'portrait'; } catch (e) { return 'portrait'; } });
    const [mapOpen, setMapOpen] = useState(false);
    const fileRef = useRef(null);
    const fetchedRef = useRef(false);
    const canEdit = !(window._wtpRoleIsReadOnly && window._wtpRoleIsReadOnly());

    const model = useMemo(() => { if (!stored || !stored.stm) return null; try { return cfpBuildModel(stored.stm, stored.summary); } catch (e) { console.error('[cfp] build', e); return null; } }, [stored]); useEffect(() => { try { localStorage.setItem('bdh-cfp-era', era); } catch (e) {} }, [era]);
    ...
    function printPdf(o) {
      ...
      try { document.title = 'BIGDREAM-CashFlow' + (model ? '-' + model.periodLabel : '') + (tabName ? '-' + tabName : ''); } catch (e) { }
    ...
    function setOrientPersist(v) { setOrient(v); try { localStorage.setItem('bdh-cfp-print-orient', v); } catch (e) { } }
      ...
      <div className="cfp-page present-page" style={pageWrap}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: C.mut, marginTop: 3 }}>BIGDREAM{model ? ' · งวด ' + model.periodLabel + ' · ' + model.txnCount + ' รายการ · ' + model.accounts.length + ' บัญชี' : ' · อัปโหลดไฟล์เพื่อเริ่ม'}</div>
            {model && (
        ...
        {model && <React.Fragment>
          ...
          <div className="no-print" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 18 }}>
            {tabs.map(([k, label]) => (
              <button key={k} onClick={() => setTab(k)} style={{ border: '1px solid ' + (tab === k ? 'transparent' : C.line), background: tab === k ? 'linear-gradient(135deg,#3ea45f,#2b8a4c)' : '#fff', color: tab === k ? '#fff' : C.mut, fontWeight: 700, fontSize: 14, padding: '9px 16px', borderRadius: 12, cursor: 'pointer', boxShadow: tab === k ? '0 6px 16px rgba(62,164,95,.3)' : 'none' }}>{label}</button>
            ))}
          </div>
