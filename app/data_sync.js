/* =====================================================================
 * data_sync.js  —  Google Sheets (CSV) sync layer for BIGDREAM Dashboard
...
(function () {
  ...
  function loadFromServer(isAuto) {
    ...
    return mapLimit(sheetOrder, SHEET_FETCH_CONC, function (n) { return fetchSheet(n); }).then(function (settled) {
      ...
      var data = {
        meta: {
          companyName: metaKV.companyName || 'BIGDREAM',
          shortName:   metaKV.shortName   || 'BIGDREAM',
  ...
  function _currentMeta(entity, oldRows, newRows) {
    var sess = null;
    try { sess = JSON.parse(localStorage.getItem('bdh-session') || 'null'); } catch (_) {}
  ...
  function _hasValidSession() {
    try {
      var s = JSON.parse(localStorage.getItem('bdh-session') || 'null');
      if (!s) return false;
