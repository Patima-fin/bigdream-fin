...
// ★ หน้านี้ถูกห่อด้วย DailyBalancePage (ท้ายไฟล์) ที่เพิ่มแท็บ "DATA BANK · บัญชีธนาคาร"
//   เข้ามา — ตัวฟอร์มบันทึกยอดเองยังเป็นโค้ดเดิมจาก BIO ทั้งดุ้น (แก้บั๊กข้ามเว็บง่าย)
function DailyBalanceEntryPage({ data, setData, toast }) {
  ...
  const saveOne = (ac) => {
    ...
    const session = JSON.parse(localStorage.getItem('bdh-session') || 'null');
...
// BIGDREAM ตัดเมนู "DATA BANK · บัญชีธนาคาร" ออกจาก sidebar แล้วเอามาไว้ที่นี่แทน —
...
const DBAL_TAB_LS_KEY = 'bdh-dbal-tab';
function DailyBalancePage({ data, setData, toast }) {
  ...
  return (
    <React.Fragment>
      <div className="tabnav" style={{ margin: '0 0 14px' }}>
