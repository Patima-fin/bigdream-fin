...
| รายงาน PEAK | หน้าในแอป | target | ทางเข้า |
|---|---|---|---|
| รายงานใบแจ้งหนี้ | ลูกหนี้คงค้าง | `iv` | `sheetToTSV` |
| รายงานบันทึกรายจ่าย | เจ้าหนี้คงค้าง | `ap` | `sheetToTSV` |
| รายงานสมุดรายวัน | ใบสำคัญจ่าย | `pv` | `sheetToTSV` |
| รายงานบัญชีแยกประเภท | งบกระทบยอดกระแสเงินสด | `gl` | **`readGL`** (ไม่ใช่ TSV) |

**จุดที่ hook ไว้ 4 ที่** (3 ตัวแรกอ่าน workbook แล้วทำเป็น TSV เหมือนกัน):
...
- `page_cf_coding.jsx` → `onGlFiles` เรียก `PeakImport.readGL(ws)` ตรง ๆ — GL ไม่ได้ป้อน
  `DataCrudPage` จึงไม่มี builder แบบ TSV (`BUILDERS.gl` ไม่มีโดยตั้งใจ)

⚠️ `detect()` หา "ชื่อรายงาน :" ด้วย `readMeta()` ที่**กวาดทุกคอลัมน์** — iv/ap/pv วางหัวรายงาน
ไว้คอลัมน์ A แต่ GL วางไว้ K-L (เพราะคอลัมน์ A เป็นหัวบล็อกบัญชี) ยึดคอลัมน์ A อย่างเดียวไม่ได้
...
- **GL**: 1 ไฟล์ = 1 บัญชีธนาคาร · หัวบล็อก A1 = `111301 - BSV001 (… - 218-2-925534 …)`
  → รหัสบัญชี/alias/ชื่อธนาคาร/เลขบัญชี · แถวแรก "ยอดยกมา" = ยอดต้นงวด · แถวท้าย "รวม"
  **`เดบิต` = เงินเข้า · `เครดิต` = เงินออก** (กลับด้านกับมุมมองใบจ่าย)
  ⚠️ รับเฉพาะรหัส `1113xx` (เงินฝากธนาคาร) — GL ของบัญชีอื่นไม่ใช่กระแสเงินสด เอาเข้ามา
  เมื่อไหร่ยอดในงบเป็นคนละเรื่องทันที · ⚠️ คอลัมน์ "คงเหลือ" ว่างทุกแถวในไฟล์จริง ห้ามพึ่ง
...
**แหล่งข้อมูล 3 ชั้น — PV ชนะ GL เสมอเมื่อเลขที่รายวันตรงกัน**

| ชั้น | มาจาก | ให้อะไร |
|---|---|---|
| ①  ใบสำคัญจ่าย | `data.pvVouchers` (นำเข้าที่หน้า PV) | ขาจ่าย + WHT + ยอดก่อนหัก |
| ②  GL | ปุ่ม **📥 นำเข้า GL** ในหน้านี้ | **ขารับ (RV) · ยอดยกมา · เลขบัญชีที่แน่นอน** |
| ③  รายการนอก PV | คีย์เองในหน้านี้ | รายการที่ไม่มีทั้งใน PV และ GL |

GL เอามาเฉพาะบรรทัดที่ `เลขที่รายวัน` **ไม่มีใน PV** — ถ้าเอามาทั้งหมดยอดจะเบิ้ล ·
บรรทัด GL ที่ตรงกับ PV ใช้เติม "บัญชีธนาคาร" ให้แถว PV (แม่นกว่า regex ที่งัดจากคำอธิบาย)

**ต่างจาก BIO อย่างไร** — BIO ใช้ EXPRESS จึงต้องไล่จับคู่บรรทัดธนาคาร ↔ ใบสำคัญจ่าย
ด้วยเลขเช็ค (โค้ดจับคู่ ~700 บรรทัด: `cfcParseBankSheet` / `cfcParseSettleReport` / `cfcMatchPv` …)
BDH ใช้ PEAK ซึ่ง GL กับสมุดรายวันใช้ `เลขที่รายวัน` (`PV-xxx`/`RV-xxx`) ตัวเดียวกัน
⇒ join ตรง ๆ ด้วยเลขเอกสาร ไม่ต้องมีตัวจับคู่แบบผ่อนปรน/เทียบยอดเลย
...
| `gl:<เลขบัญชี>:<ym>` | บรรทัด GL 1 บัญชี 1 เดือน · `opening` ติดกับบัคเก็ตเดือนแรกของช่วงเท่านั้น |
| `extra` | รายการนอก PV ที่คีย์เอง (ขารับ / ค่าธรรมเนียมที่ธนาคารหักเอง / โอนระหว่างบัญชี) |
| `manual` | `{opening:{ "<เลขบัญชี>": number }}` — ต้นงวดที่คีย์เอง (ใช้เมื่อยังไม่มี GL) |

นำเข้า GL ซ้ำ = **ทับบัคเก็ตเดิมของบัญชีนั้นทุกเดือนที่อยู่ในช่วงวันที่ของไฟล์** ก่อนเขียนใหม่ —
ไม่งั้นรายการที่ถูกลบ/แก้ใน PEAK จะค้างในระบบตลอดกาล
...
- **PEAK สมุด "จ่าย" ไม่มีขารับ** — ต้องนำเข้า GL ไม่งั้นงบติดลบทั้งเดือน (มีแถบเตือนบอก)
- **ไม่มียอดคงเหลือรายบรรทัด** ⇒ ชีต "รายละเอียดทุกรายการ" จง**ใจไม่มีคอลัมน์ "ยอดคงเหลือ"**
  ...
  ยอดต้นงวดมาจาก **"ยอดยกมา" ใน GL** ก่อน แล้วค่อยตกมาที่ค่าที่คีย์เอง
  ⚠️ ลำดับนี้ต้องเหมือนกัน **3 ที่**: ตาราง "สรุปรายบัญชี" · `openingForExport` (ไฟล์ Excel) ·
  `openingByAcct` ใน `sendToCashflow` — ตกหล่นที่ไหน ที่นั่นจะได้ต้นงวด 0 แบบเงียบ ๆ
- **ตัวเทียบยอดกับ GL ทำงานเฉพาะตอนเลือก "ทุกเดือน"** (ปลายงวดที่คิดเอง vs ปลายงวดตาม GL)
  ⚠️ effect ที่ตั้งเดือนเริ่มต้นต้องผูกกับ `allYms` เท่านั้น **ห้ามใส่ `ym` ใน deps** —
  ใส่แล้วเลือก "ทุกเดือน" (ym='') จะถูกเด้งกลับเดือนล่าสุดทันที (บั๊กนี้ยังอยู่ในของ BIO)
- **BIGDREAM มีบัญชีธนาคารบัญชีเดียว** (SCB 218-2-925534 · ผังบัญชี 111301 / BSV001)
  ⇒ **ใบ PV ที่อยู่ในช่วงวันที่ของ GL แต่ไม่มีใน GL** = ข้อมูลสองฝั่งไม่ตรงกันจริง ๆ (ไม่ใช่ "จ่ายจาก
  บัญชีอื่น") มักเป็นใบที่ถูกยกเลิก/แก้เลขใน PEAK หลังนำเข้า หรือ GL เก่ากว่า PV — แถบเตือนนับให้
  + ชี้ดูรายใบได้ · ⚠️ ต้องกรองด้วยช่วงวันที่ของ GL ไม่งั้น PV ปีที่ยังไม่ได้โหลด GL จะถูกนับว่าหาย
